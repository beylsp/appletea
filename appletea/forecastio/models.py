...
class ForecastioDataBlock(UnicodeMixin):
    ...
    - icon: machine-readable text summary of this data block (see data point,
      for an enumeration of possible values that this property may take on).
    - data: list of data point objects (see above), ordered by time, which
      together describe the weather conditions at the requested location over
      time.
...
class ForecastioDataPoint(UnicodeMixin):
    ...
    - time: UNIX time (that is, seconds since midnight GMT on 1 Jan 1970) at
      which this data point occurs. 'minutely' data points are always aligned
      to the top of the minute, 'hourly' points to the top of the hour, and
      'daily' points to midnight of the day, all according to the local time
      zone.
    ...
    - icon:machine-readable text summary of this data point, suitable for
      ...
      the following values: 'clear-day', 'clear-night', 'rain', 'snow',
      'sleet', 'wind', 'fog', 'cloudy', 'partly-cloudy-day', or
      'partly-cloudy-night'. (Developers should ensure that a sensible default
      is defined, as additional values, such as 'hail', 'thunderstorm', or
    ...
    - sunriseTime/sunsetTime (only defined on daily data points): UNIX time
      ...
      sunriseTime before and first sunset after the solar noon closest to
      locals noon on the given day.
    - moonPhase (only defined on daily data points): number representing the
      ...
      corresponds to a new moon, 0.25 to a first quarter moon, 0.5 to a full
      moon, and 0.75 to a last quarter moon. (The ranges in between these
      represent waxing crescent, waxing gibbous, waning gibbous, and waning
    ...
    - nearestStormDistance (only defined on currently data points): numerical
      ...
      value is very approximate and should not be used in scenarios requiring
      accurate results. In particular, a storm distance of zero does not
      necessarily refer to a storm at the requested location, but rather abs
      storm in the vicinity of that location.)
    - nearestStormBearing (only defined on currently data points): numerical
      value representing the direction of the nearest storm in degrees, with
      true north at 0 degrees and progressing clockwise. (If
      nearestStormDistance is zero, then this value will not be defined. The
      caveats that apply to nearestStormDistance also apply to this value.)
    - precipIntensity: numerical value representing the average expected
      ...
      (that is, assuming any precipitation occurs at all). A very rough guide
      is that a value of 0 in./hr. corresponds to no precipitation, 0.002
      in./hr. corresponds to very light precipitation, 0.017 in./hr.
      corresponds to light precipitation, 0.1 in./hr. corresponds to moderate
    ...
    - precipIntensityMax, precipIntensityMaxTime (only defined on daily data
      points): numerical values representing the maximumum expected intensity
      of precipitation (and the UNIX time at which it occurs) on the given day
      in inches/millimeters of liquid water per hour.
    ...
    - precipType: string representing the type of precipitation occurring at
      the given time. If defined, this property will have one of the following
      values: 'rain', 'snow', 'sleet' (which applies to each of freezing rain,
      ice pellets, and wintery mix), or 'hail'. (If precipIntensity is zero,
      then this property will not be defined.)
    - precipAccumulation (only defined on hourly and daily data points): the
      amount of snowfall accumulation expected to occur on the given day, in
      inches/centimeters. (If no accumulation is expected, this property will
      not be defined.)
    ...
    - temperatureMin, temperatureMinTime, temperatureMax, and
      temperatureMaxTime (only defined on daily data points): numerical values
      representing the minimum and maximumum temperatures (and the UNIX times
      at which they occur) on the given day in degrees Fahrenheit/Celsius.
    ...
    - apparentTemperatureMin, apparentTemperatureMinTime,
      ...
      maximumum apparent temperatures (and the UNIX times at which they occur)
      on the given day in degrees Fahrenheit/Celsius.
    ...
    - windBearing: numerical value representing the direction that the wind
      is coming from in degrees, with true north at 0 degrees and progressing
